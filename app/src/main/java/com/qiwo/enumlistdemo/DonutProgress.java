...
public class DonutProgress extends View {
    ...
    protected void initPainters() {
        ...
        innerBottomTextPaint.setColor(Color.argb(255,0,0,0));
    ...
    protected void onDraw(Canvas canvas) {
        ...
        {
            innerBottomTextPaint.setTextSize(innerBottomTextSize);
            float bottomTextBaseline = getHeight() / 3 - (textPaint.descent() + textPaint.ascent()) / 2;
            canvas.drawText("步数", (getWidth() - innerBottomTextPaint.measureText("步数")) / 2.0f, bottomTextBaseline, innerBottomTextPaint);
        }
        //画文本 “点击设置目标”
        {
            innerBottomTextPaint.setTextSize(innerBottomTextSize);
            float bottomTextBaseline = getHeight() * 2 / 3 - (textPaint.descent() + textPaint.ascent()) / 2;
            canvas.drawText("点击设置目标", (getWidth() - innerBottomTextPaint.measureText("点击设置目标")) / 2.0f, bottomTextBaseline, innerBottomTextPaint);
        }
        //画矩形 文本的边框
        {
            Paint rectpaint = new Paint();
            rectpaint.setStrokeWidth(1);
            rectpaint.setColor(getResources().getColor(R.color.textcolor_666));
            ...
            int padding = 20;
            float bottomTextBaseline = getHeight() * 2 / 3 - (textPaint.descent() + textPaint.ascent()) / 2;
            float left = (getWidth() - innerBottomTextPaint.measureText("点击设置目标")) / 2.0f - padding;
            float top = bottomTextBaseline - h / 2 - padding;
            float right = (getWidth() + innerBottomTextPaint.measureText("点击设置目标")) / 2.0f + padding;
            float buttom = bottomTextBaseline + padding;

            RectF rectF = new RectF(left, top, right, buttom);
            canvas.drawRoundRect(rectF, 5, 5, rectpaint);
//            canvas.drawRoundRect(left, top, right, buttom, 5, 5, rectpaint);
        }

        //中分割线1
        {
            innerBottomTextPaint.setTextSize(innerBottomTextSize);
            float bottomTextBaseline = getHeight() / 2;
            canvas.drawLine(getWidth() / 2 - getWidth() / 3, bottomTextBaseline, getWidth() / 2 - getWidth() / 3 - getWidth() / 12, bottomTextBaseline, textPaint);
